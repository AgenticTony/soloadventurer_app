...
class NotificationSchedulerService {
  ...
  Future<void> scheduleItineraryNotifications(String itineraryId) async {
    final itineraryResult = await _itineraryRepository.getItinerary(itineraryId);
  ...
  Future<void> _scheduleFlightNotifications(
    ...
    for (final flight in flights) {
      ...
      if (prefs.flightCheckInReminders) {
        final checkInTime = flightData.scheduledTime.subtract(const Duration(hours: 24));
        if (checkInTime.isAfter(DateTime.now())) {
          await _notificationRepository.schedule(
            TravelNotification(
              ...
              body: 'Your flight ${flightData.flightNumber} to ${flightData.airport} '
      ...
      if (prefs.flightGateChanges) {
        final boardingTime = flightData.scheduledTime.subtract(const Duration(hours: 1));
  ...
  Future<void> _scheduleAccommodationNotifications(
    ...
    for (final item in itinerary.items) {
      ...
      if (item case ItineraryItemHotelCheckIn(
        :final id,
        :final time,
      ...
      if (item case ItineraryItemHotelCheckOut(
        :final id,
        :final time,
        hotelName: final hotelName,
      )) {
  ...
  Future<void> _scheduleActivityReminders(
    ...
    for (final item in itinerary.items) {
      ...
      if (item case ItineraryItemActivity(
        :final id,
        :final time,
        :final name,
      ...
      if (item case ItineraryItemLunch(
        :final id,
        :final time,
        :final name,
      )) {
      ...
      if (item case ItineraryItemDinner(
        :final id,
        :final time,
        :final name,
      )) {
  ...
  Future<void> scheduleWeatherAlert({
    ...
    await _notificationRepository.schedule(
      TravelNotification(
        ...
        title: '🌤️ $condition',
        body: 'Weather alert for ${DateFormat.MMMd().format(date)}: $description',
