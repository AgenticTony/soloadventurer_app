...
class NotificationRepositoryImpl implements NotificationRepository {
  ...
  Future<NotificationStats> getStats() async {
    ...
    final totalDismissed =
        models.where((n) => n.dismissedAt != null).length;
    final unreadCount = models.where((n) => n.readAt == null).length;

    // Get pending count
    final pending = models.where((n) =>
        n.scheduledAt.isAfter(DateTime.now()) &&
        n.deliveredAt == null).length;

    // Group by category
    final byCategory = <NotificationCategory, int>{};
    for (final model in models) {
      byCategory[model.category] =
          (byCategory[model.category] ?? 0) + 1;
    }
  ...
  Future<void> cancelItineraryNotifications(String itineraryId) async {
    ...
    final toRemove = models.where((model) =>
        model.data?['itineraryId'] == itineraryId).toList();
