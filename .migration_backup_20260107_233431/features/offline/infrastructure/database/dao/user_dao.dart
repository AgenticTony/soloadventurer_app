...
class UserDao extends DatabaseAccessor<AppDatabase> {
  ...
  Future<int> updateUser(LocalUser user) async {
    return await db.users.update.replace(user);
  }
  ...
  Future<int> updateUsers(List<LocalUser> users) async {
    return await transaction(() async {
      var count = 0;
      for (final user in users) {
        count += await db.users.update.replace(user);
      }
      return count;
    });
  }
  ...
  Future<int> deleteUserById(String id) async {
    return await (db.users.delete..where((u) => u.id.equals(id))).go();
  }
  ...
  Future<int> deleteUsersByIds(List<String> ids) async {
    return await (db.users.delete..where((u) => u.id.isIn(ids))).go();
  }
  ...
  Future<int> updateUserSyncStatus(String id, bool isSynced) {
    return (db.users.update..where((u) => u.id.equals(id)))
  ...
  Future<int> markUserAsSynced(String id, DateTime lastSyncedAt) {
    return (db.users.update..where((u) => u.id.equals(id))).write(
      UsersCompanion(
  ...
  Future<int> markUsersAsSynced(List<String> ids, DateTime lastSyncedAt) {
    return (db.users.update..where((u) => u.id.isIn(ids))).write(
      UsersCompanion(
        isSynced: const Value(true),
        hasPendingChanges: const Value(false),
        lastSyncedAt: Value(lastSyncedAt),
  ...
  Future<int> updateProfileFields(String id, Map<String, dynamic> fields) {
    ...
    return (db.users.update..where((u) => u.id.equals(id))).write(companion);
  }
  ...
  Future<int> deleteAllUsers() async {
    return await db.users.delete.go();
  }
}
