...
import 'package:soloadventurer/core/services/device_info_service.dart';
...
class SecureTokenStorage {
  ...
  Future<void> initialize() async {
    try {
      ...
      if (storedKey == null) {
        ...
        _logger.info('Generated new encryption key for token storage');
      } else {
        // Use existing encryption key
        final keyBytes = base64Decode(storedKey);
        final key = encrypt.Key(Uint8List.fromList(keyBytes));
        _encrypter = encrypt.Encrypter(encrypt.AES(key));
        _logger.info('Using existing encryption key for token storage');
      }
    } catch (e, stackTrace) {
      _logger.error(
  ...
  Future<void> saveSession(AuthSession session) async {
    ...
    try {
      ...
      final encryptedAccessToken = _encrypter!.encrypt(session.accessToken, iv: _iv!);
      final encryptedIdToken = _encrypter!.encrypt(session.idToken, iv: _iv!);
      final encryptedRefreshToken = _encrypter!.encrypt(session.refreshToken, iv: _iv!);
      ...
      _logger.info('Saved encrypted session tokens');
    } catch (e, stackTrace) {
      _logger.error(
        'Failed to save session tokens',
        error: e,
        stackTrace: stackTrace,
      );
      rethrow;
  ...
  Future<AuthSession?> getSession() async {
    ...
    try {
      ...
      final encryptedAccessToken = await _secureStorage.read(key: _accessTokenKey);
      ...
      if (encryptedAccessToken == null || 
        ...
        _logger.info('No complete session found in secure storage');
        return null;
      }
      ...
      final refreshToken = _encrypter!.decrypt64(encryptedRefreshToken, iv: _iv!);
      ...
      _logger.info('Retrieved and decrypted session tokens');
      ...
      _logger.error(
        'Failed to retrieve session tokens',
        error: e,
        stackTrace: stackTrace,
      );
      return null;
  ...
  Future<void> clearSession() async {
    try {
      ...
      _logger.info('Cleared session tokens from secure storage');
    } catch (e, stackTrace) {
      _logger.error(
  ...
  Future<void> rotateEncryptionKey() async {
    try {
      ...
      _logger.info('Rotated encryption key for token storage');
    } catch (e, stackTrace) {
      _logger.error(
