...
class _SafetyHubScreenState extends ConsumerState<SafetyHubScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final safetyAsync = ref.watch(safetyNotifierProvider);
    ...
    return trustedContactsAsync.when(
      ...
      data: (trustedContacts) {
        return checkInAsync.when(
          ...
          data: (checkInData) {
            return locationSharingAsync.when(
              loading: () => const Center(
                child: CircularProgressIndicator(),
              ),
              error: (error, stack) => Center(
              ...
              data: (locationData) {
                return safetyAsync.when(
                  ...
                  data: (safetyData) {
                    final contactsCount = trustedContacts.length;
                    final upcomingCheckInsCount = checkInData.upcomingCheckIns.length;
                    final activeLocationSharesCount = locationData.activeShares.length;
                    final currentStatus = safetyData.currentStatus;
                    final hasActiveEmergency = safetyData.hasActiveEmergency;
