...
class NotificationServiceImpl implements NotificationService {
  ...
  void _onNotificationTap(NotificationResponse response) {
    ...
    if (response.payload != null) {
      try {
        ...
        // Log JSON decode errors but don't fail notification handling
  ...
  Future<bool> arePermissionsGranted() async {
    if (Platform.isAndroid) {
      ...
      // checkPermissions returns Object? (bool or Map) depending on version
      if (result is bool) return result as bool;
      // If it returns a Map, check if notifications are enabled
      if (result is Map) {
        return (result as Map)['isEnabled'] == true ||
               (result as Map)['alert'] == true ||
               (result as Map)['sound'] == true ||
               (result as Map)['badge'] == true;
      }
      return false;
    }
    return false;
  ...
  Future<NotificationDetails> _getNotificationDetails(
    ...
    final androidDetails = AndroidNotificationDetails(
      ...
      vibrationPattern: type == SafetyNotificationType.emergencySOS
          ? Int64List.fromList(SafetyNotificationConfig.vibrationPattern)
          : null,
      playSound: true,
      autoCancel: true,
    );
...
@riverpod
NotificationService notificationServiceOverride(Ref ref) {
