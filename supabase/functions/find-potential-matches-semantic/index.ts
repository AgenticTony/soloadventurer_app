...
function computeDateOverlap(
  ...
  canStart: string | null, canEnd: string | null
): number {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (targetUserId !== callerId) {
      return jsonResponse({ error: 'Can only query matches for yourself' }, 403)
    }

    const limit = Math.min(Math.max(rawLimit ?? 20, 1), 100)
    console.log(`[find-potential-matches-semantic] user=${callerId} limit=${limit}`)

    // Step 1: Call semantic RPC to get candidates
    const { data: candidates, error: rpcError } = await supabase.rpc('find_semantic_matches', {
    ...
    if (rpcError) {
      ...
      if (rpcError.message?.includes('embedding') || rpcError.message?.includes('vector')) {
        return jsonResponse({
          error: 'Profile embedding not found. Please generate your profile embedding first.',
          code: 'NO_EMBEDDING',
        }, 400)
      }
      return jsonResponse({ error: 'Failed to find semantic matches', details: rpcError.message }, 500)
    }
    ...
    const [reqProfileRes, reqTripsRes, reqActivitiesRes, canTripsRes, canActivitiesRes] = await Promise.all([
    ...
    const reqActivityRows = (reqActivitiesRes.data ?? []) as Array<{ activity_id: number; activities: { name: string } | null }>
    ...
    for (const r of reqActivityRows) {
      if (r.activities?.name) reqActivityNames.set(r.activity_id, r.activities.name)
    }

    // Index candidate trips and activities by user_id
    const canTripsMap = new Map<string, Array<{ destination_name: string | null; start_date: string | null; end_date: string | null }>>()
    for (const t of (canTripsRes.data ?? [])) {
      const arr = canTripsMap.get(t.user_id) ?? []
      arr.push({ destination_name: t.destination_name, start_date: t.start_date, end_date: t.end_date })
      canTripsMap.set(t.user_id, arr)
    }

    const canActivitiesMap = new Map<string, { ids: Set<number>; names: Map<number, string> }>()
    for (const r of ((canActivitiesRes.data ?? []) as Array<{ user_id: string; activity_id: number; activities: { name: string } | null }>)) {
      const entry = canActivitiesMap.get(r.user_id) ?? { ids: new Set<number>(), names: new Map<number, string>() }
    ...
    const matches = typedCandidates.map(candidate => {
      ...
      const canActs = canActivitiesMap.get(candidate.id) ?? { ids: new Set<number>(), names: new Map<number, string>() }

      // Date overlap — best across all trip pairings
      let bestDateScore = 0
      let bestOverlapDays = 0
      let bestTrip: { destination_name: string | null; start_date: string | null; end_date: string | null } | null = null

      for (const rt of reqTrips) {
        for (const ct of canTrips) {
          const score = computeDateOverlap(rt.start_date, rt.end_date, ct.start_date, ct.end_date)
          if (score > bestDateScore) {
            bestDateScore = score
            bestTrip = ct
            // Calculate actual overlap days
            if (rt.start_date && rt.end_date && ct.start_date && ct.end_date) {
              const os = Math.max(new Date(rt.start_date).getTime(), new Date(ct.start_date).getTime())
              const oe = Math.min(new Date(rt.end_date).getTime(), new Date(ct.end_date).getTime())
              bestOverlapDays = oe > os ? Math.round((oe - os) / (1000 * 60 * 60 * 24)) : 0
            }
          }
        }
      }
      ...
      for (const id of reqActivityIds) {
        if (canActs.ids.has(id)) {
          sharedActivities.push(reqActivityNames.get(id) ?? canActs.names.get(id) ?? `Activity ${id}`)
        }
      }

      // Destination
      const reqDest = reqTrips[0]?.destination_name ?? null
      const canDest = bestTrip?.destination_name ?? canTrips[0]?.destination_name ?? null
      ...
      return {
        ...
        _scores: { semanticScore, dateScore: bestDateScore, activityScore, destScore, ageScore },
      }
    })

    // Step 4: Sort and trim
    matches.sort((a, b) => b.composite_score - a.composite_score)
    const topMatches = matches.slice(0, limit).map(({ _scores, ...rest }) => rest)

    console.log(`[find-potential-matches-semantic] Returning ${topMatches.length} matches for user ${callerId}`)
