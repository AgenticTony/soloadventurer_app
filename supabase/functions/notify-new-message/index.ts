...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: chat } = await supabase
      .from('chats')
      .select('id, connection_id, connections!inner(requester_id, recipient_id)')
    ...
    const connection = chat.connections as { requester_id: string; recipient_id: string }
    const recipientId = message.sender_id === connection.requester_id
      ? connection.recipient_id
      : connection.requester_id

    if (!recipientId) {
      return new Response(JSON.stringify({ error: 'Could not determine recipient' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      })
    }
    ...
    if (notifError) {
      console.error('[notify-new-message] Error creating notification:', notifError)
    }
    ...
    if (tokens && tokens.length > 0) {
      console.log(`[notify-new-message] Sending push to ${tokens.length} devices`)
      ...
      if (!pushResponse.ok) {
        console.error('[notify-new-message] Push failed:', await pushResponse.text())
      }
    } else {
      console.log('[notify-new-message] No push tokens for recipient, skipping push')
    }
