...
class MockConnectivityService extends Mock implements ConnectivityService {
  ...
  Future<ConnectivityStatus> checkConnectivity() async => ConnectivityStatus(
        connectionType: _currentStatus == NetworkStatus.connected
            ? ConnectionType.wifi
            : ConnectionType.none,
  ...
  @override
  Future<NetworkStatus> checkNetworkStatus() async => _currentStatus;
...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [
        ...
        offline_providers.connectivityServiceProvider
            .overrideWithValue(connectivityService),
        tokenBlacklistManagerProvider.overrideWith(() => blacklistManager),
    ...
    // Default mock for remote refresh (returns a valid session)
  ...
  group('TokenManager - Security Features', () {
    ...
    test('should rotate tokens securely', () async {
      ...
      // Wait for any scheduled refresh to complete
      await Future.delayed(const Duration(milliseconds: 100));
      await notifier.refreshToken();

      // Assert - verify tokens were stored (either via saveAuthData or individual setters)
  ...
  group('TokenManager - Error Recovery', () {
    ...
    test('should implement exponential backoff with jitter', () async {
      ...
      // Verify that refresh attempts are made and errors are logged
      await notifier.refreshToken();
