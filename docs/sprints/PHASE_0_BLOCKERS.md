...
| **0.7 Phantom refs (NEW)**   | 4/12  | 🚨 **LAUNCH BLOCKER.** Ratchet + `.invoke()` live; **reporting fixed + UI-wired**; photos deleted; **`delete-user-account` edge fn implemented** (deploy + 👤 device test pending). Open: `shared_meetups`, `chats`, trip RPCs, journal RPC, `travel_preferences`, W.2, 👤 the 8 undeployed fns. |
