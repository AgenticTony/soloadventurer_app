...
- [x] pgTAP: another authed user CANNOT read email/phone/DOB; blocked users CANNOT read the blocker's row; women-only gating holds on direct table reads — **CLOSED 2026-07-17, all three clauses CI-green**: PII (`profiles_pii_column_privileges`, 8), block (`blocks_sever_and_gate`, 11 — both directions incl. the connected path), women-only (`profiles_policy_recursion_womenonly`, 12 — first functional proof on direct reads). Closing it required fixing a **live prod outage** found on the way: the matching policies were self/mutually recursive (`42P17` on every direct authenticated profiles/trips read in prod — armed by this story's own USING(true) drop), plus a one-directional `are_users_blocked` under caller RLS. Fixes: `20260717140000` (SECDEF predicate helpers) + `20260717150000` (codified bootstrap grants, PII column ACLs preserved). ⚠ **prod stays broken until the post-merge `db push`.**
...
- [x] ⚠ Fix `shared_meetups` — **table created 2026-07-17** (`20260717160000`) to the screen's existing payload: owner-all RLS + a `is_share_recipient` SECDEF policy so a **registered trusted contact can read shares addressed to them** (that visibility is the feature). pgTAP (9): payload lands, forged share throws, contact reads, bystander cannot. Also pins that the 150000 default-privileges mechanism auto-granted this post-migration table. **Safety — sign-off.**
...
- [x] Fix `notify-new-message` — **fixed 2026-07-17.** Three defects, not one: the guard rejected every trigger payload (`chat_id` doesn't exist → 400 on EVERY message); the `chats` join was phantom; and `profiles.first_name` doesn't exist either (sender always "Someone"). Now: recipient = `messages.receiver_id` (it was on the row all along — the join was never needed), sender = `full_name ?? username`, payload carries `connectionId` (which the mobile tap-handler already navigates on). **Redeploy after CI.** 👤 Follow-up: deep-link opens ChatScreen with empty `chatId` extra (pre-existing — the old payload could never arrive at all); resolve chat from connectionId in ChatScreen.
- [x] Fix web's trip RPCs — **repointed 2026-07-17** to direct `trips` table queries under the existing owner-CRUD + public-read RLS (no migration needed; the RPCs were imagined, the policies were real). Pagination contract preserved (inclusive `range` = peek row). The api tests — which mocked the phantom RPCs and stayed green while every trip op failed live — now pin the table name and query shapes. Web 482/482.
- [x] Fix `get_entries_near_location` — **RPC created 2026-07-17** (`20260717170000`) to the datasources' exact signature; PostGIS `ST_DWithin` on geography (real meters), self-only guard raises `P0001` (the `find_semantic_matches` pattern), SECURITY INVOKER so journal RLS applies. pgTAP: Malmö entry within 10km returned, Stockholm excluded, cross-user query throws.
- [x] Fix `travel_preferences` — **table created 2026-07-17** (`20260717180000`) mirroring the Freezed model field-for-field; owner-only RLS (private L1 matching input); `UNIQUE(user_id)` keeps the sync pipeline idempotent. pgTAP: owner CRUD, one-row-per-user, other users read nothing. Note: **no UI writes preferences yet** — the offline pipeline (upload_sync + conflict_resolver + EntityType) was fully built against this table; now it has a real destination for when the feature ships.
...
- [x] ⚠ **NEW (2026-07-17): fix `delete-user-account`** — was invoked by `auth_remote_data_source_impl.dart:614` but existed **nowhere in `supabase/functions/`**: account deletion was broken (GDPR right + app-store requirement). **Edge function implemented same day** to the client's documented contract (POST + JWT → `{success:true}`/`{error}`): verifies the caller's JWT (self-delete only; no request body trusted), clears the **only two no-cascade FKs** (`reports.resolved_by` → NULL; the user's `safety_alerts` rows → deleted), purges storage under `${userId}/` in `avatars`/`journal-photos`/`journal-videos` (both upload services verified to use that flat prefix), then `auth.admin.deleteUser` → profiles cascade. **⚠ Embedded policy call needing sign-off: the user's `safety_alerts` rows (location points) are ERASED, overriding the table's immutable-audit-log design — erasure right > audit retention, pre-launch.** 👤 Remaining: deploy + a real delete-account test on device.
- [x] **Deploy the 8 undeployed edge functions — DONE 2026-07-17** (Anthony's go: "we need to fix these"). All 8 deployed via `supabase functions deploy`; **verified live: prod now runs 12/12 ACTIVE** (was 4/12). Six are fully operational (they need only platform-injected `SUPABASE_URL`/`SERVICE_ROLE_KEY`). **Two need 👤 secrets before their gated paths work** (`supabase secrets list` verified neither is set):
  - `verify-with-onfido` → `ONFIDO_API_TOKEN` + `ONFIDO_WEBHOOK_SECRET` (Anthony's Onfido account)
  - `process-checkin` → `SYSTEM_SECRET`
  Until those are set, calls that hit the secret-gated paths return 500 instead of the old 404 — deployed-but-unkeyed, not fixed-end-to-end. `delete-user-account` (the 9th) was deployed **after** CI's Edge Functions Validation passed on it — **prod is now 13/13 ACTIVE**, verified twice (the first deploy attempt failed behind a CLI telemetry error and was caught by re-listing; never trust the deploy message without the list).
- [x] Each fix lands with a test that would **fail against a wrong name** — held for every fix: reports/blocks/shared_meetups/travel_preferences/get_entries_near_location have pgTAP against a real database; web's block + trip suites pin table names and query shapes. **The ratchet BASELINE is now EMPTY** — every historical phantom is fixed, and any new one fails the build with no grace period.
...
| 0.5 Profiles RLS             | 6/6   | ✅ **COMPLETE 2026-07-17** — box 4 proven (PII + block + women-only, 31 pgTAP assertions). Repo-complete; ⚠ prod needs the post-merge `db push` (recursion fix). |
...
| **0.7 Phantom refs (NEW)**   | 11/12 | ✅ **All phantoms FIXED 2026-07-17; ratchet baseline EMPTY.** Remaining box: W.2 generated types (web lane — the durable web-side gate). ⚠ sign-off + `db push` (3 new migrations) + `notify-new-message` redeploy pending. |
