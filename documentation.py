...
def process_markdown_file(file_path, source_name, supabase_client):
    """Process a markdown file and store it in the database"""
    try:
        ...
        # Split content into chunks (you may want to implement a more sophisticated chunking strategy)
        chunk_size = 1000
        ...
        for i, chunk in enumerate(chunks):
            ...
            supabase_client.table("site_pages").insert(data).execute()
        
        return True, f"Successfully processed {len(chunks)} chunks from {file_path}"
...
def documentation_tab(supabase_client):
    ...
    doc_tabs = st.tabs(["Pydantic AI Docs", "Custom Documentation", "Future Sources"])
    ...
    with doc_tabs[1]:
        st.subheader("Custom Documentation")
        st.markdown("""
        This section allows you to add custom documentation from markdown files.
        The processor will:
        
        1. Read the markdown files
        2. Split content into chunks
        3. Generate embeddings for each chunk
        4. Store the chunks in the Supabase database
        
        Currently supported documentation:
        - AWS Cognito (@AWS_Cognito.md)
        - Flutter (@Flutter.md)
        ...
        if not supabase_url or not supabase_key:
            ...
            # Create columns for the documentation types
            col1, col2, col3 = st.columns(3)
            
            with col1:
                if st.button("Process AWS Cognito Docs"):
                    with st.spinner("Processing AWS Cognito documentation..."):
            ...
            with col2:
                if st.button("Process Flutter Docs"):
                    with st.spinner("Processing Flutter documentation..."):
                        success, message = process_markdown_file(
                            "docs/@Flutter.md",
                            "flutter_docs",
                            supabase_client
            ...
            with col3:
                if st.button("Process Riverpod Docs"):
                    with st.spinner("Processing Riverpod documentation..."):
                        success, message = process_markdown_file(
                            "docs/@Riverpod.md",
                            "riverpod_docs",
                            supabase_client
            ...
            # Add a button to clear all custom documentation
            if st.button("Clear All Custom Documentation"):
                with st.spinner("Clearing custom documentation..."):
                    try:
                        # Delete records for all custom documentation sources
                        sources = ["aws_cognito_docs", "flutter_docs", "riverpod_docs"]
                        for source in sources:
                            supabase_client.table("site_pages").delete().eq("metadata->>source", source).execute()
                        st.success("✅ Successfully cleared all custom documentation.")
                    except Exception as e:
                        st.error(f"❌ Error clearing custom documentation: {str(e)}")
            
            # Display database statistics
            st.subheader("Database Statistics")
            try:
                # Query counts for each documentation type
                sources = {
                    "AWS Cognito Docs": "aws_cognito_docs",
                    "Flutter Docs": "flutter_docs",
                    "Riverpod Docs": "riverpod_docs"
                }
                
                cols = st.columns(len(sources))
                for i, (name, source) in enumerate(sources.items()):
                    result = supabase_client.table("site_pages").select("count", count="exact").eq("metadata->>source", source).execute()
                    count = result.count if hasattr(result, "count") else 0
                    cols[i].metric(name, count)
                
                # Add a button to view the data
                if st.button("View Indexed Data", key="view_custom_data"):
                    # Query a sample of the data from all custom sources
                    sample_data = supabase_client.table("site_pages").select("url,title,content,chunk_number").in_("metadata->>source", list(sources.values())).limit(10).execute()
