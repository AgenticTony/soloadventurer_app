      ...
      for (var day = 1; day <= itinerary.numberOfDays; day++) {
        ...
        if (hasRain) {
          ...
          final outdoorActivities = items.where((item) {
            return switch (item) {
              ItineraryItemActivity(:final name) =>
                _isOutdoorActivityFromName(name),
              _ => false,
            };
          }).toList();
          ...
          if (outdoorActivities.isNotEmpty) {
            suggestions.add(OptimizationSuggestion(
              ...
              description:
                  '${outdoorActivities.length} outdoor activit${outdoorActivities.length == 1 ? 'y' : 'ies'} affected. '
        ...
        if (item case ItineraryItemActivity(:final name, :final time)) {
          ...
          if (peakInfo.hours.contains(currentTime)) {
            ...
            suggestions.add(OptimizationSuggestion(
              ...
              timeSaved:
                  const Duration(minutes: 30), // Time saved waiting in line
            ));
          }
        }
      }
